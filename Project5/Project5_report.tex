...
We will once again, like in project 4, use the Metropolis algorithm to solve our quantum mechanical system. We first have to start the electrons at an arbitrary position. We let the electron start at the position $x,y,z \in [-1,1]$, which we get from a normal distribution. 

Then, for every Monte Carlo cycle, we have to give the electrons a new position determined by $\mathbf{R}' = \mathbf{R} + \delta\times s$, where $\mathbf{R} = (\mathbf{r}_1, \mathbf{r}_2, ..., \mathbf{r}_N)$, $s$ is a random number generated from a normal distribution such that $s\in [-1,1]$ and $\delta$ is a small step length. Note that $\delta \times s$ is different for every component, e.g
\begin{align*}
...
We can then proceed with the Metropolis algorithm, which is given as follows: 
\begin{itemize}
\item  Calculate $w = P(\mathbf{R}')/P(\mathbf{R}) = |\Psi(\mathbf{R}')|^2/|\Psi(\mathbf{R})|^2$, where $\Psi$ is the wave function we will consider for the calculation (i.e the trial wave functions $\psi_{T1}$ and/or $\psi_{T2}$ mentioned previously). Pick a random number $p\in [0,1]$, also given from a normal distribution function. We now have to consider these two cases
...
We then use the new or old position (depending on whatever case above hits) to calculate the local energy $E_L = \frac{1}{\Psi} H_0\Psi$. Alternatively, we can instead use our analytical expressions of the local energies to calculate the energy. This will save a lot of CPU time because we do not have to calculate the Laplace operator $\nabla^2$. Once that is done, we repeat the process.
...
\begin{center}
	...
	\end{tabular}
\caption{The energy, variance and $\alpha$ values near the minimum point. Data set for $\omega = 1$.}
