...
Let us not plot the energy (expectation value) and variance as a function of of the variational parameter $\alpha$. We can use these determine the optimal value of $\alpha$, which we can use to compare with our results from project 2. One thing to note is that the optimal variational parameter may be different for different frequencies $\omega$.
...
\subsubsection{Optimal $\alpha$ and $\beta$ parameters}
Like the first trial wave function, we would like to find the optimal value of the variational parameters $\alpha$ and $\beta$. 
\FloatBarrier
