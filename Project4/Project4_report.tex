...
The algorithm also worked very well when we increased the lattice size. We saw that the computed energy were indeed larger for a larger temperature. This agrees with our physical intuition, where a system with a higher temperature is more energetic than a system with lower temperature.
...
Although, one can probably bump the number Monte Carlo cycles for $L=140$ and even reduce the temperature step to obtain a more consistent answer, but that will increase the computation time quite considerably.
\FloatBarrier
